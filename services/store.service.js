...
class StoreService {
  ...
  getStoreDetail = async (storeId) => {
    try {
      ...
      if (!oneStoreData) {
        throw errorHandler.nonExistStore;
      }
      // console.log(oneStoreData);
  ...
  search = async (searchKeyword) => {
    try {
      let stores = new Set();
      let storeIds = [];
      const result = await this.storeRepository.searchMenu(searchKeyword);
      for (let i = 0; i < result.length; i++) {
        stores.add(result[i].dataValues.store_id);
      }
      for (let value of stores.values()) {
        storeIds.push(value);
      }
      const allStoreData = await this.storeRepository.searchStore(searchKeyword, storeIds);
